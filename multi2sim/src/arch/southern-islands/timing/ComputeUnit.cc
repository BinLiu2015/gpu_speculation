			...
			WavefrontPool *wavefront_pool)
{
	...
	for (int i = 0; i < max_wavefronts_per_wavefront_pool; i++)
	{
		...
		unsigned index = (fetch_buffer->getLastIssuedWavefrontIndex() + i + 1)
		...
		if (branch_unit.isValidUop(uop))
		{
			if (branch_unit.canIssue())
			{
				...
				{
					IssueToExecutionUnit(fetch_buffer, &branch_unit, index,
							instructions_issued_in_current_wavefront);
					if (instructions_issued_in_current_wavefront == 1)
					{
		...
		if (scalar_unit.isValidUop(uop))
		{
			if (scalar_unit.canIssue())
			{
				...
				{
					IssueToExecutionUnit(fetch_buffer, &scalar_unit, index,
							instructions_issued_in_current_wavefront);
					if (instructions_issued_in_current_wavefront == 1)
					{
		...
		if (simd_units[id]->isValidUop(uop))
		{
			if (simd_units[id]->canIssue())
			{
				if (!fetch_buffer->getSIMDUnitIssuedFlag())
				{
					IssueToExecutionUnit(fetch_buffer, simd_units[id].get(),
		...
		if (lds_unit.isValidUop(uop))
		{
			if (lds_unit.canIssue())
			{
				...
				{
					IssueToExecutionUnit(fetch_buffer, &lds_unit, index,
							instructions_issued_in_current_wavefront);
					if (instructions_issued_in_current_wavefront == 1)
					{
		...
		WavefrontPool *wavefront_pool)
{
	...
	for (int i = 0; i < max_wavefronts_per_wavefront_pool; i++)
	{
		...
		// entry
		if (fetch_buffer->IsLastFetchedEntryFull(index) == 1 && i == 0)
			index++;
		...
		{
			fetch_buffer->setLastFatchedWavefrontIndex(index);
			fetch_buffer->setLastFetchedEntryFull(true);
			break;
		}
