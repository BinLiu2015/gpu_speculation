...
Timing::Timing() : comm::Timing("SouthernIslands")
{
	...
	// Hint info
	// Read all the hints to hint buffer
	if (!Timing::hint_file.empty())
	{
		std::ifstream hint;
		hint.open(Timing::hint_file);
		int temp;
		hint_format temp_data;
		hint >> temp;
		hint >> temp_data.warp_change;
		hint >> temp_data.mode;
		hint >> temp_data.offset1;
		hint >> temp_data.offset2;

		while(!hint.eof())
		{
			instruction_hint.insert(std::make_pair(temp,temp_data));
			hint >> temp;
			hint >> temp_data.warp_change;
			hint >> temp_data.mode;
			hint >> temp_data.offset1;
...
void Timing::RegisterOptions()
{
	...
	// Option --si-hint <file>
	command_line->RegisterString("--si-hint <file>", hint_file,
			"Shader hint file generated by compiler for gpu scheduler.");

	// Option --si-issue-mode <int>
	command_line->RegisterUInt32("--si-issue-mode <mode>", Timing::issue_mode,
			"Issue scheduler mode. 1 default, 0 speculation mode.");
...
void Timing::DumpReport() const
{
	...
	{ 
		...
		report << misc::fmt("WavefrontCount = %lld\n",
				compute_unit->num_mapped_wavefronts);
		...
		report << misc::fmt("Speculation Instructions = %lld\n",
				compute_unit->num_total_speculation_instructions);
		report << misc::fmt("Speculation ScalarInstructions = %lld\n",
				compute_unit->num_scalar_speculation_instructions);
		report << misc::fmt("Speculation ScalarMemoryInstructions = %lld\n",
				compute_unit->num_scalar_memory_speculation_instructions);
		report << misc::fmt("Speculation BranchInstructions = %lld\n",
				compute_unit->num_branch_speculation_instructions);
		report << misc::fmt("Speculation SIMDInstructions = %lld\n",
				compute_unit->num_simd_speculation_instructions);
		report << misc::fmt("Speculation VectorMemInstructions = %lld\n",
				compute_unit->num_vector_memory_speculation_instructions);
		report << misc::fmt("Speculation LDSInstructions = %lld\n",
				compute_unit->num_lds_speculation_instructions);
		report << misc::fmt("Num Total Execution Mode = %lld\n",
				compute_unit->num_total_speculation_mode);
		report << misc::fmt("Long latency Stall Cycles = %lld\n",
				compute_unit->long_latency_stall_cycles + getCycle()-
						compute_unit->last_issue_cycle);
		//report << misc::fmt("Long latency Stall Cycles = %lld\n",
		//		compute_unit->long_latency_stall_cycles + compute_unit->last_complete_cycle
		//				- compute_unit->last_issue_cycle);
		report << misc::fmt("pool issue cycle = %lld\n",
				compute_unit->wavefront_pool_issued_cycles);

		report << misc::fmt("pool cycle = %lld\n",
					compute_unit->wavefront_pool_cycles);
